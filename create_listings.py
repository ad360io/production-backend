...
for temp_adspace in adspaceList:
    date1 = random.randint(1, 28)
    date2 = random.randint(1,28)
    to_month = random.randint(1,12)
    date_from = datetime(2017,11,date1,5,5,5,5)
    date_to = datetime(2018,to_month,date2, 5,5,5,5)
    temp_adspaceListing = RequestForAdv(name = temp_adspace.name + ' adspace listing',currency = random.choice(currencyList),ask_date_from = date_from , ask_date_to = date_to, cpi = random.uniform(10,50), cpm = random.uniform(5,50), msg = 'This is a sample adspace listing for the adspace' + temp_adspace.name)
    temp_adspaceListing.save()
    temp_adspaceListing.adsp.add(temp_adspace)
